...
func TestAudioOutputMailboxBounds100HzFramesAndCarriesDiscontinuity(t *testing.T) {
	var output outputBase
	output.mailbox.paused = true
	frameBytes := make([]byte, 160*2)
	delivered := make([]AudioFrame, 0, mailboxDataCapacity+1)

	for index := 0; index < mailboxDataCapacity; index++ {
		frame := AudioFrame{
			Data:              frameBytes,
			PTS:               time.Duration(index) * 10 * time.Millisecond,
			SampleRateHz:      16000,
			Channels:          1,
			SamplesPerChannel: 160,
			Discontinuity:     output.discontinuity(false),
		}
		if !output.mailbox.postData(func() { delivered = append(delivered, frame) }) {
			t.Fatal("10 ms audio frame rejected before mailbox capacity")
		}
	}
	if output.mailbox.postData(func() {}) {
		t.Fatal("100 Hz audio frames exceeded the bounded mailbox")
	}
	output.markDropped()
	output.mailbox.unpause()
	output.mailbox.waitIdle()

	next := AudioFrame{
		Data:              frameBytes,
		PTS:               mailboxDataCapacity * 10 * time.Millisecond,
		SampleRateHz:      16000,
		Channels:          1,
		SamplesPerChannel: 160,
		Discontinuity:     output.discontinuity(false),
	}
	if !output.mailbox.postData(func() { delivered = append(delivered, next) }) {
		t.Fatal("post-overflow audio frame rejected")
	}
	output.mailbox.waitIdle()

	if len(delivered) != mailboxDataCapacity+1 {
		t.Fatalf("delivered %d audio frames", len(delivered))
	}
	for index, frame := range delivered {
		if len(frame.Data) != 320 || frame.SamplesPerChannel != 160 ||
			frame.PTS != time.Duration(index)*10*time.Millisecond {
			t.Fatalf("invalid 10 ms frame at %d: %+v", index, frame)
		}
	}
	if !delivered[len(delivered)-1].Discontinuity {
		t.Fatal("first deliverable frame after overflow did not carry discontinuity")
	}

	if err := output.mailbox.beginClose(); err != nil {
		t.Fatal(err)
	}
	output.mailbox.finishClose()
	if output.mailbox.postData(func() {}) {
		t.Fatal("audio callback crossed closed mailbox")
	}
}
